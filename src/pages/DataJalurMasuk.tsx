...
export const DataJalurMasuk: React.FC = () => {
  const [jalurMasuk, setJalurMasuk] = useState<any[]>([]);
  ...
  const fetchJalurMasukLocal = async () => {
    setLoading(true);
    setError('');
    ...
    try {
      ...
      if (data.jalur_masuk) {
        setJalurMasuk(data.jalur_masuk);
      }
    } catch (err: any) {
      setError('Gagal mengambil data jalur masuk lokal: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRefresh = async () => {
    ...
    try {
      const data = await fetchWithAuth('/api/neofeeder', {
        ...
        body: JSON.stringify({ action: 'GetJalurMasuk' })
      });
      
      if (data.error_code === 0 && data.data) {
        setJalurMasuk(data.data);
      } else {
  ...
  const handleSave = async () => {
    if (jalurMasuk.length === 0) return;
    setSaveLoading(true);
    setSaveSuccess(false);
    setError('');
    try {
      const data = await fetchWithAuth('/api/jalur-masuk/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ items: jalurMasuk })
      });
      if (data.success) {
        setSaveSuccess(true);
  ...
  useEffect(() => {
    fetchJalurMasukLocal();
  }, []);

  const filteredJalurMasuk = jalurMasuk.filter(w => 
    w.nama_jalur_masuk?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    w.id_jalur_masuk?.toString().toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[#111827]">Data Jalur Masuk</h1>
          <p className="text-[#6B7280] mt-1 text-sm">Daftar jalur masuk dari PDDIKTI Neofeeder</p>
        </div>
        <div className="flex gap-3">
          ...
          <button
            onClick={handleSave}
            disabled={loading || saveLoading || jalurMasuk.length === 0}
      ...
      <div className="bg-white rounded-xl border border-[#E5E7EB] overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.1)]">
        <div className="px-6 py-5 border-b border-[#E5E7EB] flex items-center justify-between bg-[#F9FAFB]">
          <div className="flex items-center space-x-3 text-[#111827]">
            ...
            <h3 className="m-0 text-base font-semibold">Daftar Jalur Masuk</h3>
          </div>
        ...
        ) : filteredJalurMasuk.length === 0 ? (
          <div className="p-12 text-center text-[#6B7280]">
             <AlertCircle className="h-12 w-12 mx-auto mb-4 text-[#D1D5DB]" />
             <p>Tidak ada data jalur masuk ditemukan.</p>
          </div>
        ) : (
          <div className="overflow-x-auto h-[600px] overflow-y-auto">
            <table className="w-full text-sm border-collapse relative">
              <thead className="bg-[#F9FAFB] sticky top-0 z-10 shadow-sm">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-[#6B7280] border-b border-[#E5E7EB]">ID Jalur Masuk</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-[#6B7280] border-b border-[#E5E7EB]">Nama Jalur Masuk</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-[#F3F4F6]">
                {filteredJalurMasuk.map((d, i) => (
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#1F2937]">{d.id_jalur_masuk}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-[#374151]">{d.nama_jalur_masuk}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="px-6 py-4 border-t border-[#E5E7EB] flex justify-between items-center bg-[#F9FAFB]">
          <span className="text-[13px] text-[#6B7280]">Total {filteredJalurMasuk.length} Data Jalur Masuk</span>
        </div>
      </div>
    </div>
  );
