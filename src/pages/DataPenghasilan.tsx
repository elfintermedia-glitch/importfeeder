...
export const DataPenghasilan: React.FC = () => {
  ...
  const fetchPenghasilanLocal = async () => {
    setLoading(true);
    setError('');
    setSaveSuccess(false);
    try {
      const data = await fetchWithAuth('/api/penghasilan', {
        method: 'GET'
      });
      if (data.penghasilan) {
        setPenghasilan(data.penghasilan);
      }
    } catch (err: any) {
      setError('Gagal mengambil data penghasilan lokal: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRefresh = async () => {
    ...
    try {
      const data = await fetchWithAuth('/api/neofeeder', {
        ...
        body: JSON.stringify({ action: 'GetPenghasilan' })
      });
      
      if (data.error_code === 0 && data.data) {
        setPenghasilan(data.data);
      } else {
  ...
  const handleSave = async () => {
    if (penghasilan.length === 0) return;
    setSaveLoading(true);
    setSaveSuccess(false);
    setError('');
    try {
      const data = await fetchWithAuth('/api/penghasilan/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ items: penghasilan })
      });
      if (data.success) {
        setSaveSuccess(true);
  ...
  useEffect(() => {
    fetchPenghasilanLocal();
  }, []);

  const filteredPenghasilan = penghasilan.filter(w => 
    w.nama_penghasilan?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    w.id_penghasilan?.toString().toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[#111827]">Data Penghasilan</h1>
          <p className="text-[#6B7280] mt-1 text-sm">Daftar penghasilan dari PDDIKTI Neofeeder</p>
        </div>
        <div className="flex gap-3">
          ...
          <button
            onClick={handleSave}
            disabled={loading || saveLoading || penghasilan.length === 0}
      ...
      <div className="bg-white rounded-xl border border-[#E5E7EB] overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.1)]">
        <div className="px-6 py-5 border-b border-[#E5E7EB] flex items-center justify-between bg-[#F9FAFB]">
          <div className="flex items-center space-x-3 text-[#111827]">
            ...
            <h3 className="m-0 text-base font-semibold">Daftar Penghasilan</h3>
          </div>
        ...
        ) : filteredPenghasilan.length === 0 ? (
          <div className="p-12 text-center text-[#6B7280]">
             <AlertCircle className="h-12 w-12 mx-auto mb-4 text-[#D1D5DB]" />
             <p>Tidak ada data penghasilan ditemukan.</p>
          </div>
        ) : (
          <div className="overflow-x-auto h-[600px] overflow-y-auto">
            <table className="w-full text-sm border-collapse relative">
              <thead className="bg-[#F9FAFB] sticky top-0 z-10 shadow-sm">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-[#6B7280] border-b border-[#E5E7EB]">ID Penghasilan</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-[#6B7280] border-b border-[#E5E7EB]">Nama Penghasilan</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-[#F3F4F6]">
                {filteredPenghasilan.map((d, i) => (
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#1F2937]">{d.id_penghasilan}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-[#374151]">{d.nama_penghasilan}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="px-6 py-4 border-t border-[#E5E7EB] flex justify-between items-center bg-[#F9FAFB]">
          <span className="text-[13px] text-[#6B7280]">Total {filteredPenghasilan.length} Data Penghasilan</span>
        </div>
      </div>
    </div>
  );
